...
@foreach(
    ...
    <section>
        <div class="header">
            ...
            <div>
                <b>Noms et Prénoms :</b> {{$etudiant["etudiant"]->nom}} {{$etudiant["etudiant"]->prenom}}
                <b id="Matricule">Matricule :</b>{{$etudiant['etudiant']->matricule}}
                <br><i>Surname and Name : </i><i id="Matricule1">Registration N°:</i></p>
            </div>
            <div>
                <b>Né(e) le :</b>   {{$etudiant['etudiant']->datenaiss}}
                <b id="Matricule">A : </b>  {{$etudiant['etudiant']->lieu}}
                <br><i>Born on : </i><i id="Matricule3">At:</i></p>
            </div>
            <div>
                <b>Domaine</b> :</b> {{$etudiant['inscription']->filiere->departement->nomdepartement}}
                <br><i>Domain : </i>
            </div>
            <div>
                <b>Niveau :</b> {{$etudiant['inscription']->niveau->niveau}}
                <b id="Matricule4">Fillière :</b> {{$etudiant['inscription']->filiere->intitulefiliere}}
                <br><i>Level : </i><i id="Matricule5">Discipline :</i></p>
            </div>
            <div>
                <b>Spécialité :</b> {{$etudiant['inscription']->filiere->specialite->nomspecialite}}
                <b id="Matricule2">Année Academique :</b> {{$etudiant['inscription']->anneAcademique->annee}}
        ...
        <div id="legend">
            ...
            <div id="legend1">
                Crédits Capitalisés : <b>{{$etudiant['credit']->cca}} </b>/{{$etudiant['credit']->totalCredit}} (<b>{{$etudiant['credit']->pourcentage}} %</b>)<br>
                Moyenne Générale Pondérée (MGP): <b>{{$etudiant['mgp_decision']->mgp}}</b> /4 <br>
                Décision : <b>{{$etudiant['mgp_decision']->decision}}</b>
            </div>
        </div>
